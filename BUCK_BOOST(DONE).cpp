...
///PI control///
int	Expectation = 48;
float Expect_Digital_Value = 0 ;
int AVGa[256],ana[256],err[256],p[256],kii[256],pi[256];
int analogy=0, count = 0;
...
void main(void)
{
   ...
   InitAdc();         // init the ADC
	...
	AdcRegs.ADCSOC0CTL.bit.CHSEL = 1;       		//設置 ADCINA 0 轉換
...
//初始化pwm//
void InitEPwmTimer(void){
	EPwm1Regs.TBPRD = 500; // Period = 1500 TBCLK counts
	                             // TBCLK = 1/60M  T=50us
...
void PIcontrol(void){
	     ...
	     verr= ( Expect_Digital_Value - AVGan);
	     kp = (verr * 0.1 *8192  );

	     ki = (verrR + verr) ;
	     kiR = (kiR + (ki  ));

	 	 if(kiR >=  ((long int)495 << 13) ){kiR = ((long int)495 << 13) ; }
	     if(kiR <= -((long int)495 << 13) ){kiR = -((long int)495 << 13); }

	     verrR = verr;

	     pierr= ( (long int)(kp + kiR) ) >> 13;
...
void swPWM (void){
	swpwm = GpioDataRegs.GPADAT.bit.GPIO6;        		//設置pwm開關
	  if( swpwm==1 )  // 取樣 110v  1126 (55)&& GpioDataRegs.GPADAT.bit.GPIO9 == 0
		{
		 EPwm1Regs.CMPA.half.CMPA = pierr;	// Compare A = pierr TBCLK counts
