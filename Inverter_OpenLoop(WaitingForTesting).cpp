...
#define DutyCycle1 0.5
#define DutyCycle2 0.5
#define FREQ_TBPRD	( (int)30000000 / Frequency )
#define Half_FREQ_TBPRD		FREQ_TBPRD/2
#define CMPA_Value1  ( FREQ_TBPRD - (DutyCycle1*2*FREQ_TBPRD) ) // for CAU_SET && CAD_CLEAN
#define CMPA_Value2  ( FREQ_TBPRD - (DutyCycle2*2*FREQ_TBPRD) ) // for CAU_SET && CAD_CLEAN
...
__interrupt void cpu_timer0_isr(void) {
   ...
   if(GpioDataRegs.GPADAT.bit.GPIO4 ==1 && GpioDataRegs.GPADAT.bit.GPIO8 == 1 ){
	  ...
	  //	CMPA2Value  = 	Half_FREQ_TBPRD	 + ( ( (long int)( vout*( sintable1[ADCCount]*(-1)  ) - IoCalVlue ) ) >>9 ) ;
	   	...
	   	EPwm1Regs.CMPA.half.CMPA =  CMPA_Value1 ; //FREQ_TBPRD	- CMPA1Value ;
	   	EPwm2Regs.CMPA.half.CMPA =  CMPA_Value2; //FREQ_TBPRD	- CMPA2Value ;
